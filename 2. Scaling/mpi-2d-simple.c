...
int main(int argc, char * argv[]) {
	...
	for (k=0;k<MAX_ITERATIONS;k++) {
		if (coords[1] > 0) {
			MPI_Sendrecv(&u_k[1+mem_size_x], local_nx, MPI_DOUBLE, down_neighbour, 0, &u_k[1], local_nx, MPI_DOUBLE,
					down_neighbour, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
		if (coords[1] < dims[1]-1) {
			MPI_Sendrecv(&u_k[(local_ny*mem_size_x)+1], local_nx, MPI_DOUBLE, up_neighbour, 0, &u_k[((local_ny+1)*mem_size_x)+1], local_nx,
					MPI_DOUBLE, up_neighbour, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		}
		...
		if (coords[0] > 0) {
			...
			MPI_Sendrecv(send_buffer_left, local_ny, MPI_DOUBLE, left_neighbour, 0, recv_buffer_left, local_ny,
								MPI_DOUBLE, left_neighbour, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			for (i=1;i<=local_ny;i++) {
		...
		if (coords[0] < dims[0]-1) {
			...
			MPI_Sendrecv(send_buffer_right, local_ny, MPI_DOUBLE, right_neighbour, 0, recv_buffer_right, local_ny,
											MPI_DOUBLE, right_neighbour, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			for (i=1;i<=local_ny;i++) {
