...
int main(int argc, char * argv[]) {
	...
	MPI_Request requests[8], reduction_request;

	int i,j,k,r;
	for (i=0;i<8;i++) requests[i]=MPI_REQUEST_NULL;
	...
	for (k=0;k<MAX_ITERATIONS;k++) {
		...
		if (coords[1] < dims[1]-1) {
			MPI_Isend(&u_k[(local_ny*mem_size_x)+1], local_nx, MPI_DOUBLE, up_neighbour, 0, MPI_COMM_WORLD, &requests[2]);
			MPI_Irecv(&u_k[((local_ny+1)*mem_size_x)+1], local_nx, MPI_DOUBLE, up_neighbour, 0, MPI_COMM_WORLD, &requests[3]);
		}
		...
		if (coords[0] > 0) {
			...
			MPI_Isend(send_buffer_left, local_ny, MPI_DOUBLE, left_neighbour, 0, MPI_COMM_WORLD, &requests[4]);
			MPI_Irecv(recv_buffer_left, local_ny, MPI_DOUBLE, left_neighbour, 0, MPI_COMM_WORLD, &requests[5]);
		}
		...
		if (coords[0] < dims[0]-1) {
			...
			MPI_Isend(send_buffer_right, local_ny, MPI_DOUBLE, right_neighbour, 0, MPI_COMM_WORLD, &requests[6]);
			MPI_Irecv(recv_buffer_right, local_ny, MPI_DOUBLE, right_neighbour, 0, MPI_COMM_WORLD, &requests[7]);
		}

		tmpnorm=0.0;
		for (j=2;j<local_ny;j++) {
			for (i=2;i<local_nx;i++) {
				tmpnorm=tmpnorm+pow(u_k[i+(j*mem_size_x)]*4-u_k[(i-1)+(j*mem_size_x)]-
						u_k[(i+1)+(j*mem_size_x)]-u_k[i+((j-1)*mem_size_x)]-u_k[i+((j+1)*mem_size_x)], 2);
			}
		}
		...
		MPI_Iallreduce(&tmpnorm, &rnorm, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD, &reduction_request);
		...
		MPI_Wait(&reduction_request, MPI_STATUS_IGNORE);
