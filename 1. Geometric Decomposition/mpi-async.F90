...
contains
  ...
  subroutine run_solver()
    ...
    integer :: i, k, ierr, size, myrank, local_nx, requests(4)
    ...
    do k=0, MAX_ITERATIONS
      if (myrank .gt. 0) then
        call mpi_isend(u_k(1), 1, MPI_DOUBLE, myrank-1, 0, MPI_COMM_WORLD, requests(1), ierr)
        call mpi_irecv(u_k(0), 1, MPI_DOUBLE, myrank-1, 0, MPI_COMM_WORLD, requests(2), ierr)
      end if
      if (myrank .lt. size-1) then
        call mpi_isend(u_k(local_nx), 1, MPI_DOUBLE, myrank+1, 0, MPI_COMM_WORLD, requests(3), ierr)
        call mpi_irecv(u_k(local_nx+1), 1, MPI_DOUBLE, myrank+1, 0, MPI_COMM_WORLD, requests(4), ierr)
      end if
